...
.controller('ChatsCtrl', ['$scope', '$firebase', '$rootScope', '$localstorage',
	function( $scope, $firebase, $rootScope, $localstorage){
		...
		var ref = new Firebase('https://realtimechat-b3b07.firebaseio.com');
		...
		$scope.sendChat = function(chat){
          ...
          //구글로 로그인
        else if(authData.loginType=="google"){
          $scope.chats.$add({
...
.controller('AccountCtrl', function($scope, $rootScope, $localstorage) {
  ...
  $scope.loginWithFacebook =function () {
    firebase.auth().signInWithPopup(facebookprovider).then(function(result) {
      ...
      $rootScope.loginType="facebook";
      $localstorage.setObject("authData",token);
      // The signed-in user info.
      var user = result.user;
      // ...
    },{remember:"default"}).catch(function(error) {
